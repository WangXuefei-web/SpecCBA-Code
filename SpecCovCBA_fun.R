...
# The main functions are SpecCov(sample, alpha, K) and SpecCBA(sample, alpha, K)
...
### Function for applying SpecCov
SpecCov <- function(sample, alpha, K){ 
  # output NMI, HamAcc, loglik
  A <- sample$adjT
  N <- dim(A)[1]
  L <- dim(A)[3]
...
SpecCBA <- function(sample, alpha, K){ 
  # output NMI, HamAcc, loglik
  A <- sample$adjT
  N <- dim(A)[1]
  L <- dim(A)[3]
...
SpecCBA.app <- function(A,alpha.seq=seq(-1,5,0.2),K){
  # output alpha, label.hat
  result.seq <- array(NA, dim=c(length(alpha.seq),2)) # loglik alpha
  alpha.i <- 1
  for (alpha in alpha.seq){
    re.SpecCBA <- SpecCBA.A(A, alpha, K)
    result.seq[alpha.i,] <- c(re.SpecCBA$loglik,alpha)
    alpha.i <- alpha.i+1
  }
  ...
  label.hat <- SpecCBA.A(A, alpha.opt, K)$label.hat
