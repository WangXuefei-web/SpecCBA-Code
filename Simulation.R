...
# Simulation 1: Properties of SpecCBA alpha=opt,fixed, (a) varying the number of layers L; (b) varying the number of nodes N; (c) varying the sparsity parameter rho

# Compared Methods: SpecCBA vs MeanAdj(only mean), OLMF(centered square), SOS-debiased, HOOI(rTensor)
...
### Simulation 1: Properties of SpecCBA alpha=opt,fixed ### 
...
for (simul in 1:simul.max){
  print(simul)
  for (L in L.seq){
  sample <- GenerateComplementaryMLSBM(N, M, L, K, d, r )
  ...
  for (alpha in alpha.seq){
    ...
    result.seq[alpha.i,] <- c(alpha,L,re.SpecCBA$HamAcc,re.SpecCBA$loglik) # alpha L HamAcc loglik
    alpha.i <- alpha.i+1
  }
...
for (simul in 1:simul.max){
  ...
  for (N in N.seq){
    ...
    for (alpha in alpha.seq){
      re.SpecCBA <- SpecCBA(sample, alpha, K)
      result.seq[alpha.i,] <- c(alpha,N,re.SpecCBA$HamAcc,re.SpecCBA$loglik) # alpha N HamAcc loglik
      alpha.i <- alpha.i+1
    }
    max.id <- which.max(result.seq[,4])
    result.seq <- as.data.frame(result.seq)
...
for (simul in 1:simul.max){
  ...
  for (rho in rho.seq){
    ...
    for (alpha in alpha.seq){
      re.SpecCBA <- SpecCBA(sample, alpha, K)
      result.seq[alpha.i,] <- c(alpha,rho,re.SpecCBA$HamAcc,re.SpecCBA$loglik) # alpha rho HamAcc loglik
      alpha.i <- alpha.i+1
    }
    max.id <- which.max(result.seq[,4])
    result.seq <- as.data.frame(result.seq)
...
png("result/simul1.png", height=600,width=1500, res=168)
print(p1)
dev.off()

# Compared Methods: SpecCBA vs MeanAdj(only mean), OLMF(centered square), SOS-BA, HOOI(rTensor)
# Simulation 2: complementary information alpha=opt, (a) varying the number of layers L; (b) varying the sparsity parameter rho
# (a) varying the number of layers L; 
set.seed(123)
N <- 600 #the number of nodes
M <- 1 #types of networks
...
simulresult <- data.frame("L"=NA,"SpecCBA"=NA,"MeanAdj"=NA,"OLMF"=NA,"SOSBA"=NA,"HOOI"=NA)
simulresult <- simulresult[-1,]

for (simul in 1:simul.max){
  print(simul)
  for (L in L.seq){
    sample <- GenerateComplementaryMLSBM(N, M, L, K, d, r ) # indirected
    SpecCBA.re <- SpecCBA.fun(sample,alpha.seq,K)
    MeanAdj.re <- MeanAdj.fun(sample,K)
    OLMF.re <- OLMF.fun(sample,K)
    SOSBA.re <- SOSBA.fun(sample,K)
    HOOI.re <- HOOI.fun(sample,K)
    HamAcc.re <- data.frame('L'=L,'SpecCBA'=SpecCBA.re$HamAcc,'MeanAdj'=MeanAdj.re$HamAcc,
                            'OLMF'=OLMF.re$HamAcc,'SOSBA'=SOSBA.re$HamAcc,'HOOI'=HOOI.re$HamAcc)
    simulresult <- dplyr::bind_rows(simulresult,HamAcc.re)
  }
}

mean.re <- simulresult %>% group_by(L) %>% summarise(SpecCBA=mean(SpecCBA),MeanAdj=mean(MeanAdj),
                                                     OLMF=mean(OLMF),SOSBA=mean(SOSBA),HOOI=mean(HOOI))
#save(mean.re,file="result/simul2_varyL.Rda")
load(file="result/simul2_varyL.Rda")
# plot result
mean.re.long <- mean.re %>% gather(key="Method",value="HamAcc",SpecCBA:HOOI)
colorbar5 <- c("#984EA3","#FF7F00","#377EB8","#4DAF4A", "#E41A1C")
p2.L <- ggplot(data=mean.re.long,aes(x=L,y=HamAcc,color=Method,shape=Method,linetype=Method))+
  geom_line(linewidth=0.9)+
  geom_point(size=1.5)+
  scale_shape_manual(values = c(16:18,7,15))+
  scale_color_manual(values = colorbar5)+
  theme_gray()+
  #theme(legend.position = "bottom")+
...
simulresult <- data.frame("rho"=NA,"SpecCBA"=NA,"MeanAdj"=NA,"OLMF"=NA,"SOSBA"=NA,"HOOI"=NA)
simulresult <- simulresult[-1,]
...
for (simul in 1:simul.max){
  ...
  for (rho in rho.seq){
    ...
    SOSBA.re <- SOSBA.fun(sample,K)
    HOOI.re <- HOOI.fun(sample,K)
    HamAcc.re <- data.frame('rho'=rho,'SpecCBA'=SpecCBA.re$HamAcc,'MeanAdj'=MeanAdj.re$HamAcc,
                            'OLMF'=OLMF.re$HamAcc,'SOSBA'=SOSBA.re$HamAcc,'HOOI'=HOOI.re$HamAcc)
    simulresult <- dplyr::bind_rows(simulresult,HamAcc.re)
  }
}

mean.re <- simulresult %>% group_by(rho) %>% summarise(SpecCBA=mean(SpecCBA),MeanAdj=mean(MeanAdj),
                                                     OLMF=mean(OLMF),SOSBA=mean(SOSBA),HOOI=mean(HOOI))
...
simulresult <- data.frame("K"=NA,"SpecCBA"=NA,"MeanAdj"=NA,"OLMF"=NA,"SOSBA"=NA,"HOOI"=NA)
simulresult <- simulresult[-1,]
...
for (simul in 1:simul.max){
  ...
  for (K in K.seq){
    ...
    d <- rho*N #average degree (a L-dimensional vector)}
    sample <- GenerateMMSBM(N, M, L, K, d, r ) # indirected
    SpecCBA.re <- SpecCBA.fun(sample,alpha.seq,K)
    MeanAdj.re <- MeanAdj.fun(sample,K)
    OLMF.re <- OLMF.fun(sample,K)
    SOSBA.re <- SOSBA.fun(sample,K)
    HOOI.re <- HOOI.fun(sample,K)
    HamAcc.re <- data.frame('K'=K,'SpecCBA'=SpecCBA.re$HamAcc,'MeanAdj'=MeanAdj.re$HamAcc,
                            'OLMF'=OLMF.re$HamAcc,'SOSBA'=SOSBA.re$HamAcc,'HOOI'=HOOI.re$HamAcc)
    simulresult <- dplyr::bind_rows(simulresult,HamAcc.re)
  }
}

mean.re <- simulresult %>% group_by(K) %>% summarise(SpecCBA=mean(SpecCBA),MeanAdj=mean(MeanAdj),
                                                     OLMF=mean(OLMF),SOSBA=mean(SOSBA),HOOI=mean(HOOI))
...
p2.K <- ggplot(data=mean.re.long,aes(x=K,y=HamAcc,color=Method,shape=Method,linetype=Method))+
  ...
  theme_gray()+
  #theme(legend.position = "bottom")+
  theme(legend.position= c(0.25,0.3),legend.key.size = unit(0.7,'cm'))+
...
p2 <- ggarrange(p2.rho,p2.L,p2.K,ncol = 3, nrow = 1,common.legend = T)
png("result/simul2.png", height=600,width=1500, res=168)
print(p2)
dev.off()